...
namespace custom_types {
    ...
    void setupTypeHierarchy(Il2CppClass* klass, Il2CppClass* baseClass) {
        ...
        if (baseClass) {
            if (!baseClass->initialized_and_no_error) {
                ...
                i2c::functions::Class_Init(baseClass);
            }
        ...
        if (bt) {
            ...
            if (cb) {
                ...
                if (!bt->initialized) {
                    ...
                    i2c::functions::Class_Init(bt);
                }
                vtableSize = bt->vtable_count;
                if (vtableSize == 0) {
                    logger.warn(
                        "You ({}::{}) are attempting to inherit from a type that has no vtable entries ({}::{}). This is almost always a mistake. Did you mean to declare the base type as a custom base instead?",
                        namespaze(),
                        name(),
                        bt->namespaze,
                        bt->name
    ...
    void TypeRegistration::createClass() {
        ...
        if (cb) {
            ...
            if (loadingTypes.contains(cb)) {
                custom_types::logger.critical(
                    "Circular dependency! {}::{} (base type of: {}::{}) is already being loaded!", cb->namespaze(), cb->name(), namespaze(), name()
                );
                SAFE_ABORT();
            }
        ...
        custom_types::logger.debug(
            "Creating type: {}::{} with: {} methods, {} static fields, {} fields!",
            namespaze(),
            name(),
            getMethods().size(),
            getStaticFields().size(),
            getFields().size()
        );
        ...
        if (intfs.size() != all_intfs.size()) {
            // scan for any interfaces the modder might have missed, since on quest all parent interfaces have to be implemented for things to work
            // properly
            for (auto parent : all_intfs) {
                if (!parent)
                    continue;
                if (!i2c::functions::class_is_interface(parent))
                    continue;
                auto itr = std::find(intfs.begin(), intfs.end(), parent);
                if (itr == intfs.end()) {
                    custom_types::logger.warn(
                        "Parent interface {}::{} did not appear in requested interfaces of type {}::{}, but was found in a recursive search!",
                        parent->namespaze,
                        parent->name,
                        namespaze(),
                        name()
                    ...
                    custom_types::logger.warn(
                        "This interface was subsequently added to the list of implemented interfaces, make sure to also implement parent interfaces!"
                    );
        ...
        for (size_t i = 0; i < all_intfs.size(); i++) {
            custom_types::logger.debug("Implementing for {}: {}", fullName, i2c::class_standard_name(all_intfs[i], true));
    ...
    void TypeRegistration::populateFields() {
        ...
        if (staticFields.size() > 0 || static_fields_size() > 0) {
            ...
            // See: Class.cpp:1012
            SAFE_ABORT("SAY NO TO STATIC FIELDS!");
        }
        ...
        if (static_fields_size() > 0) {
            ...
            if (i2c::functions::has_gc_funcs) {
                static_fields() = reinterpret_cast<char*>(i2c::functions::GarbageCollector_AllocateFixed(static_fields_size(), nullptr));
            } else {
    ...
    bool TypeRegistration::checkVirtualsForMatch(
        MethodRegistrator* info, std::string_view namespaze, std::string_view name, std::string_view methodName, int paramCount
    ) {
        ...
        if (info->virtualMethod()) {
            ...
            // pure virtuals (abstract) have a vMethod->klass (& other data) that is null, but those can never be Finalize. therefore we just skip the
            // rest of the checks if !k
            return k && namespaze == k->namespaze && name == k->name && methodName == info->csharpName() &&
                   (paramCount >= 0 ? info->params_size() == paramCount : true);
        }
        return false;
    }
    ...
    void TypeRegistration::getVtable(std::vector<VirtualInvokeData>& vtable, std::vector<Il2CppRuntimeInterfaceOffsetPair>& offsets) {
        ...
        if (baseT) {
            ...
            if (!cb && !baseT->initialized_and_no_error) {
                ...
                i2c::functions::Class_Init(baseT);
            }
            ...
            for (auto m : methods) {
                ...
                if (vMethod != nullptr) {
                            ...
                            for (auto inter : infs) {
                                if (vMethod->klass == inter) {
                                    ...
                                    if (inter->method_count < vMethod->slot) {
                                        ...
                                        logger.critical(
                                            "Interface: {} method count: {} < virtual data: {} slot: {}",
                                            fmt::ptr(inter),
                                            inter->method_count,
                                            fmt::ptr(vMethod),
                                            vMethod->slot
                    ...
                    if (!set) {
                        ...
                        logger.critical(
                            "Method: {} ({}) needs virtual_data: {} which requires type: {} which does not exist!",
                            m->csharpName(),
                            fmt::ptr(m),
                            fmt::ptr(vMethod),
                            fmt::ptr(vMethod->klass)
                        );
    ...
    void TypeRegistration::populateMethods() {
        ...
        for (size_t i = 0; i < vtable.size(); i++) {
            ...
            if (vtable[i].method == nullptr && vtable[i].methodPtr == nullptr) {
                custom_types::logger.critical(
                    "Type: {}::{} has Vtable index: {} has null method or method pointer! Ensure you implement the interface entirely (and do not use any nullptrs!)",
                    namespaze(),
                    name(),
                    i
                );
        ...
        k->methods = reinterpret_cast<MethodInfo const**>(calloc(k->method_count, sizeof(MethodInfo*)));
