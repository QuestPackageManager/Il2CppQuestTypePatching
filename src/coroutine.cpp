...
namespace custom_types::Helpers {
    ...
    Il2CppObject* ResetableCoroutine::get_Current() {
        ...
        //     custom_types::logger.debug("Unity asked for resettable value, we have {} {} {}", fmt::ptr(current.instance),
        //     il2cpp_utils::ClassStandardName(current.instance->klass, true),
    ...
    Il2CppObject* StandardCoroutine::get_Current() {
        ...
        //     auto containsInterface = std::find(current.instance->klass->implementedInterfaces, current.instance->klass->implementedInterfaces +
        //     current.instance->klass->interfaces_count, enumeratorTKlass);

        //     custom_types::logger.debug("Unity asked for value, we have {} {} {}", fmt::ptr(current.instance),
        //     il2cpp_utils::ClassStandardName(current.instance->klass, true),
    ...
    void CoroutineHelper::EnsureCoroutines() {
        ...
        if (!initialized) {
            custom_types::logger.debug(
                "Calling EnsureCoroutines... Attempting to "
                "initialize coro custom types"
            );
            custom_types::Register::ExplicitRegister({&__registration_instance_ResetableCoroutine_7, &__registration_instance_StandardCoroutine_8});
            initialized = true;
        }
    }
}  // namespace custom_types::Helpers
