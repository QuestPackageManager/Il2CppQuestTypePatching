...
struct std::hash<std::pair<std::string, std::string>> {
    size_t operator()(std::pair<std::string, std::string> const& item) const noexcept {
...
namespace custom_types {
    ...
    class Register {
       ...
       private:
        ...
        static void addToMapping(TypeRegistration const* itr) {
       ...
       public:
        ...
        CUSTOM_TYPES_EXPORT static void AutoRegister() {
            ...
            for (auto actual : toRegister) {
                ...
                custom_types::logger.debug(
                    "Registered registration: {}, {}::{} klass: {}, {}::{}, image: {}",
                    fmt::ptr(actual),
                    actual->namespaze(),
                    actual->name(),
                    fmt::ptr(actual->klass()),
                    actual->klass()->namespaze,
                    actual->klass()->name,
                    fmt::ptr(actual->klass()->image)
                );
